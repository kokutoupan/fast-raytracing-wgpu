...
impl State {
    ...
    fn save_screenshot(&self, src_texture: &wgpu::Texture) {
        ...
        if receiver.recv().unwrap().is_ok() {
            ...
            let width = self.config.width;
            let height = self.config.height;
            let padded_bytes_per_row = self.screenshot_padded_bytes_per_row as usize;
            ...
            std::thread::spawn(move || {
                ...
                #[cfg(not(feature = "ai-denoise"))]
                {
                    ...
                    let temp_buffer = image::ImageBuffer::<image::Rgba<u8>, Vec<u8>>::from_raw(
                        width, height, rgba_data,
                    )
                    .expect("Dimension mismatch");

                    // 2. 簡易ブラー (sigma=1.0〜2.0程度なら十分高速)
                    // ※完全にブラーもいらないならここも削除すればもっと速い
                    // final_image_buffer = image::imageops::blur(&temp_buffer, 1.5);
